package com.sz7road.eat.web.utils;
...
public final class WebUtil {
     ...
     * 保存当前用户
     */
    public static final void saveCurrentUser(TBizUser user) {
     ...
     * 获取当前用户
     */
    public static final TBizUser getCurrentUser() {
        Subject currentUser = SecurityUtils.getSubject();
        if (null != currentUser) {
            try {
                Session session = currentUser.getSession();
                if (null != session) {
                    return (TBizUser) session.getAttribute(Constants.CURRENT_USER);
                }
